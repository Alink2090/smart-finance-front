...
export default function Login() {
  // ✅ Fix 1 — destructurer sendLoginOtp ET login depuis useAuth
  ...
  // Step 1 — vérifie credentials → envoie OTP
  const handleFormSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      // ✅ Fix 2 — sendLoginOtp du context, pas authAPI directement
  ...
  const handleOtpSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      // ✅ login(email, password, otp) — les 3 paramètres
        ...
        {step === 'form' && (
          <>
            <div style={{ textAlign: 'center', marginBottom: 32 }}>
              ...
              <h1 style={{ fontSize: 24, fontWeight: 800, color: 'var(--text)', letterSpacing: '-.03em', marginBottom: 6 }}>
                Welcome back
              </h1>
            ...
            <div style={{ textAlign: 'center', marginTop: 24, fontSize: 14, color: 'var(--text2)' }}>
              ...
              <Link to="/register" style={{ color: 'var(--accent2)', fontWeight: 600, textDecoration: 'none' }}>Create one</Link>
            </div>
          </>
        )}
