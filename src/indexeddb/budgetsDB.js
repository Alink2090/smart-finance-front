...
export async function getBudgets(userId) {
  ...
  return all.filter(b => b.sync_status !== 'pending_delete')
}
...
export async function updateBudget(localIdOrServerId, data) {
  let record = await get(STORE, localIdOrServerId)
  if (!record) {
    const all = await getAllByIndex(STORE, 'server_id', String(localIdOrServerId))
    record = all[0]
  }
  if (!record) throw new Error(`Budget ${localIdOrServerId} introuvable`)
  const updated = {
    ...record, ...data,
...
export async function deleteBudget(localIdOrServerId, userId) {
  let record = await get(STORE, localIdOrServerId)
  if (!record) {
    const all = await getAllByIndex(STORE, 'server_id', String(localIdOrServerId))
    record = all[0]
...
export async function cacheBudgets(userId, serverItems) {
